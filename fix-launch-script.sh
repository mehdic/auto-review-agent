#!/bin/bash
# Fix the launch-agents-from-spec.sh to actually use the loop scripts

echo "🔧 Fixing launch-agents-from-spec.sh to use the loop scripts..."

# Backup the original
cp launch-agents-from-spec.sh launch-agents-from-spec.sh.bak

# Find the line numbers for the sections we need to replace
# We need to replace the reviewer section that has the long while loop

# Create a temporary fixed version
cat > launch-agents-from-spec-fixed.sh << 'EOF'
#!/bin/bash
set -e
...
# Make it executable
chmod +x launch-agents-from-spec-fixed.sh

# Replace the original
mv launch-agents-from-spec-fixed.sh launch-agents-from-spec.sh

echo "✅ Fixed launch-agents-from-spec.sh!"
echo ""
echo "The launch script now properly uses the loop scripts instead of inline commands."
echo ""
echo "Test it now:"
